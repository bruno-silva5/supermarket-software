...
public class AtualizarCaixa extends JFrame{
	...
	public AtualizarCaixa(CaixaModel caixa) {
		id = caixa.getId();
		setUndecorated(true);
		...
		JLabel lblAtualizarCaixa = new JLabel("Atualizar Caixa");
		lblAtualizarCaixa.setFont(new Font("Arial", Font.PLAIN, 26));
		lblAtualizarCaixa.setForeground(new Color(53,152,220));
		lblAtualizarCaixa.setHorizontalAlignment(SwingConstants.CENTER);
		lblAtualizarCaixa.setBounds(0, 12, 366, 51);
		getContentPane().add(lblAtualizarCaixa);
		...
		JLabel lblCadastrar = new JLabel("Atualizar");
		...
		txtEmail.setText(caixa.getEmail());
		passwordField.setText(caixa.getSenha());
	...
	private void cadastrar() {
		if(!(txtNome.getText().equals("") || txtEmail.getText().equals("") || new String(passwordField.getPassword()).equals(""))) {
			...
			if(CaixaController.update(caixa)) {
				txtNome.setText("");
				txtEmail.setText("");
